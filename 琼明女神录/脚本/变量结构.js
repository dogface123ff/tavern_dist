...
export const Schema = z.object({
  ...
  世界: z.object({
    ...
    在场角色: z.record(z.string().describe('角色名'), z.object({ 穿着衣装: z.string().prefault(''), 当前状态: z.string().prefault('') })).prefault({
      裴语涵: {
  ...
  主角: z.object({
    ...
    储物袋: z.object({
      ...
      物品: z.record(z.string().describe('物品名'), z.object({ 分类: z.string().describe('丹药/材料/杂物').prefault(''), 描述: z.string().prefault(''), 数量: z.coerce.number().prefault(0) })).prefault({}),
      装备栏: z.object({
        本命武器: z.string().prefault('名剑“羡鱼”')
      })
    })
  }),
  关系列表: z.record(z.string().describe('角色名'), z.object({
    修为: z.string().prefault(''),
    好感度: z.coerce.number().transform(v => _.clamp(v, -50, 200)).prefault(0),
    关系态度: z.string().prefault(''),
    重要事件: z.string().prefault('')
  })).prefault({}),
  事件列表: z.record(z.string().describe('事件名'), z.object({
