...
const MOUNT_ID = 'qj-mvu-editor-launcher';
const EDITOR_SOURCE = 'qj-mvu-editor';
const FRAME_TRANSITION = 'width 240ms cubic-bezier(.22, 1, .36, 1), height 240ms cubic-bezier(.22, 1, .36, 1), border-color 180ms ease, border-radius 180ms ease, box-shadow 180ms ease';
const FRAME_MARGIN = 8;
const COLLAPSED_FRAME_SIZE = 44;
const tavernWindow = (window.parent === window ? window : window.parent) as TavernHost;
const tavernDocument = tavernWindow.document;
let unbindFrameMessages: (() => void) | null = null;

function getFrameTransition() {
  return tavernWindow.matchMedia?.('(prefers-reduced-motion: reduce)').matches ? 'none' : FRAME_TRANSITION;
}

function clampManualFramePosition(frame: HTMLIFrameElement, open: boolean) {
  const left = Number.parseFloat(frame.style.left);
  const top = Number.parseFloat(frame.style.top);
  if (!Number.isFinite(left) || !Number.isFinite(top)) return;

  const targetWidth = open ? Math.min(680, Math.max(FRAME_MARGIN * 2, tavernWindow.innerWidth - 28)) : COLLAPSED_FRAME_SIZE;
  const targetHeight = open ? Math.min(580, Math.max(FRAME_MARGIN * 2, tavernWindow.innerHeight - 28)) : COLLAPSED_FRAME_SIZE;
  const maxLeft = Math.max(FRAME_MARGIN, tavernWindow.innerWidth - targetWidth - FRAME_MARGIN);
  const maxTop = Math.max(FRAME_MARGIN, tavernWindow.innerHeight - targetHeight - FRAME_MARGIN);
  const clampedLeft = Math.min(Math.max(left, FRAME_MARGIN), maxLeft);
  const clampedTop = Math.min(Math.max(top, FRAME_MARGIN), maxTop);
...
function setFrameMode(frame: HTMLIFrameElement, open: boolean) {
  Object.assign(frame.style, {
    width: open ? 'min(680px, calc(100vw - 28px))' : `${COLLAPSED_FRAME_SIZE}px`,
    height: open ? 'min(580px, calc(100vh - 28px))' : `${COLLAPSED_FRAME_SIZE}px`,
    border: open ? '1px solid rgba(229, 235, 241, 0.38)' : '0',
    borderRadius: open ? '12px' : '50%',
    boxShadow: open ? '0 26px 68px rgba(0, 0, 0, 0.52)' : 'none',
  });
  clampManualFramePosition(frame, open);
  frame.style.transform = '';
  frame.style.willChange = 'width, height';
  frame.dataset.open = String(open);
}

function bindFrameMessages(frame: HTMLIFrameElement) {
  const messageHandler = (event: MessageEvent) => {
    if (event.source !== frame.contentWindow) return;
    const message = event.data as { source?: string; type?: string } | null;
    if (!message || message.source !== EDITOR_SOURCE) return;
    if (message.type === 'open') setFrameMode(frame, true);
    if (message.type === 'close') setFrameMode(frame, false);
  };
  tavernWindow.addEventListener('message', messageHandler);
  return () => tavernWindow.removeEventListener('message', messageHandler);
}

function bindFrameDrag(frame: HTMLIFrameElement, childDocument: Document) {
  let dragging = false;
  let moved = false;
  let pointerId = -1;
  let startX = 0;
  ...
  let trailingDragClickUntil = 0;

  const clamp = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max);

  const renderPosition = () => {
    animationFrame = null;
    frame.style.transform = `translate3d(${nextLeft - originLeft}px, ${nextTop - originTop}px, 0)`;
  };

  const queuePosition = () => {
    if (animationFrame !== null) return;
    animationFrame = tavernWindow.requestAnimationFrame(renderPosition);
  };

  const updatePosition = (event: PointerEvent) => {
    const deltaX = event.screenX - startX;
    const deltaY = event.screenY - startY;
    if (!moved && deltaX * deltaX + deltaY * deltaY < 25) return;
    moved = true;
    nextLeft = clamp(originLeft + deltaX, 8, maxLeft);
    nextTop = clamp(originTop + deltaY, 8, maxTop);
  };
  ...
  const finish = (event?: PointerEvent) => {
    ...
    dragging = false;
    childDocument.removeEventListener('pointermove', move);
    childDocument.removeEventListener('pointerup', finish);
    childDocument.removeEventListener('pointercancel', finish);
    if (animationFrame !== null) {
      tavernWindow.cancelAnimationFrame(animationFrame);
      animationFrame = null;
    }
    renderPosition();
    ...
    frame.style.willChange = 'width, height';
    try {
      if (pointerId >= 0 && captureTarget?.hasPointerCapture?.(pointerId)) captureTarget.releasePointerCapture(pointerId);
    } catch (error) {
      console.debug('[琼明 MVU 编辑器] 指针捕获释放失败。', error);
    }
    pointerId = -1;
    captureTarget = null;
    if (handle) handle.style.cursor = '';
    if (moved && handle?.matches('.seal')) trailingDragClickUntil = Date.now() + 600;
    handle = null;
    frame.style.transition = getFrameTransition();
  };
  ...
  const start = (event: PointerEvent) => {
    ...
    if (event.button !== 0 || !target?.closest('[data-drag-handle]')) return;
    if (target.closest('[data-editor-close], input, select, textarea')) return;
    handle = target.closest('[data-drag-handle]') as HTMLElement | null;
    if (handle?.matches('.header') && target.closest('button')) return;
    const rect = frame.getBoundingClientRect();
    originLeft = rect.left;
    originTop = rect.top;
    nextLeft = rect.left;
    nextTop = rect.top;
    maxLeft = Math.max(8, tavernWindow.innerWidth - rect.width - 8);
    maxTop = Math.max(8, tavernWindow.innerHeight - rect.height - 8);
    startX = event.screenX;
    startY = event.screenY;
    trailingDragClickUntil = 0;
    dragging = true;
    moved = false;
    pointerId = event.pointerId;
    frame.style.transition = 'none';
    frame.style.left = `${originLeft}px`;
    frame.style.top = `${originTop}px`;
    frame.style.right = 'auto';
    frame.style.bottom = 'auto';
    frame.style.transform = 'translate3d(0, 0, 0)';
    frame.style.willChange = 'transform';
    if (handle) handle.style.cursor = 'grabbing';
    captureTarget = target instanceof HTMLElement ? target : handle;
    try { captureTarget?.setPointerCapture?.(event.pointerId); } catch (error) { console.debug('[琼明 MVU 编辑器] 指针捕获不可用。', error); }
    childDocument.addEventListener('pointermove', move, { passive: false });
    childDocument.addEventListener('pointerup', finish);
    childDocument.addEventListener('pointercancel', finish);
  };

  const blockTrailingDragClick = (event: MouseEvent) => {
    if (!trailingDragClickUntil) return;
    if (Date.now() > trailingDragClickUntil) {
      trailingDragClickUntil = 0;
      return;
    }
    trailingDragClickUntil = 0;
  ...
  childDocument.addEventListener('pointerdown', start);
  childDocument.addEventListener('click', blockTrailingDragClick, true);
}

function mount() {
  unbindFrameMessages?.();
  tavernDocument.getElementById(MOUNT_ID)?.remove();

  const frame = tavernDocument.createElement('iframe');
  frame.id = MOUNT_ID;
  frame.title = '琼明女神录 MVU 变量编辑器';
  ...
  Object.assign(frame.style, {
    ...
    boxShadow: 'none',
    display: 'block',
    transition: getFrameTransition(),
    willChange: 'width, height, transform',
    contain: 'layout paint',
  });
  frame.setAttribute('allowtransparency', 'true');
  frame.style.setProperty('background-color', 'transparent', 'important');
  setFrameMode(frame, false);
  tavernDocument.body.append(frame);
  unbindFrameMessages = bindFrameMessages(frame);
  frame.addEventListener('load', () => {
    const childDocument = frame.contentDocument;
    if (!childDocument) return;
    bindFrameDrag(frame, childDocument);
  });
...
async function start() {
  if (typeof tavernWindow.waitGlobalInitialized === 'function') await tavernWindow.waitGlobalInitialized('Mvu');
  mount();
}

const jquery = tavernWindow.$ ?? (window as typeof window & { $?: JQueryStatic }).$;
if (jquery) jquery(() => void start().catch(error => console.error('[琼明 MVU 编辑器] 初始化失败。', error)));
else void start().catch(error => console.error('[琼明 MVU 编辑器] 初始化失败。', error));

$(window).on('pagehide', () => {
  unbindFrameMessages?.();
  unbindFrameMessages = null;
  tavernDocument.getElementById(MOUNT_ID)?.remove();
});
